...
final class MemoryPolynomialState(C, size_t N, size_t P, size_t Mf, size_t Mp)
{
    alias F = typeof(C.re);
    ...
    C[1 + 1 + (P-1)*(Mf+1+Mp)][N] state;
    C[1 + 1 + (P-1)*(Mf+1+Mp)][N] weight;
    ...
    void update(C c)
    {
        ...
        foreach(p; 2 .. P+1)
            pmemory[0][p-2] = cabs2^^p;

        immutable xk = xmemory[Mf];
        state[0][0] = complexZero!C + 1;    // 1 + 0i
        state[0][1] = xk;

        foreach(p; 2-2 .. P+1-2) foreach(m; 0 .. Mp+Mf+1){
            state[0][2 + p*(Mp+Mf+1) + m] = xk * pmemory[m][p];
        }
