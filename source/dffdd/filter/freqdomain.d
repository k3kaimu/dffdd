...
if(isBlockConverter!(Dist, C, C[]) && isFrequencyDomainMISOStateAdapter!(StateAdapter, C))
{
        ...
        foreach(testcase; [1, 0])
        {
            ...
            foreach(f; 0 .. _nFFT * _nOS)
                foreach(p; 0 .. _distorter.outputDim)
                    weight[f][p] = testfilter._stateAdapter._states[f].weight[0][p];
            */
            ...
            if(_doComplexSelect && testcase == 0)
            {
                ...
                expectedPowers = calculateExpectedPower(model, genSignal, weight, _iqRX);
            }
            else
            {
                // H_{p,q}[f] を取得する
    ...
    C[][] estimateCFR(M, Signals)(M model, Signals delegate(M) genSignal, C iqRX = C(0, 0))
    {
        ...
        foreach(i; 0 .. 1024 / spb){
            ...
            // 受信信号から受信機IQインバランスの影響を抜く
            foreach(ref e; testRX)
                e = (e - iqRX * conj(e)) / (1 - iqRX.sqAbs);
    ...
    Tuple!(real[][], real[]) calculateExpectedPower(M, Signals)(M model, Signals delegate(M) genSignal, in C[][] weight, C iqRX = C(0, 0))
    {
        auto signals = genSignal(model);
        C[] testTX = new C[this.inputBlockLength],
            testRX = new C[this.inputBlockLength],
        ...
        return tuple(powerOfSI, powerOfRCV);
    }
...
// final class IQInversionCascadeFilter(C, size_t P)
// {
//     alias LSStateAdapter = FrequencyDomainParallelHammersteinStateAdapter!(C, LSAdapter);
//     alias WLFilter = FrequencyDomainHammersteinFilter!(C, Distorter!(C, x => x, x => conj(x)), LSStateAdapter);


//     this(BaseFilter baseFilter, size_t nSym4IQEst, size_t nSC4IQEst)
//     {
//         immutable nSym = (_nFFT + _nCP) * _nOS;

//         foreach(p; 0 .. P) _buffer4PA[p] = new C[nSym];
//         _buffer4Regenerator = new C[P][](nSym);
//     }
...
//     {
//         immutable a = _distorter.inputBlockLength,
//                   b = (_nFFT + _nCP) * _nOS * 2;        // 2つのSWAPシンボルを加算するために2倍にする
...
//         {
//             auto ips = input[i*nSym .. (i+1)*nSym];
//             auto dss = desired[i*nSym .. (i+1)*nSym];
//             auto ers = errors[i*nSym .. (i+1)*nSym];
//             applyForEachSymbol!doLearning(ips, dss, ers);
//             ++_nWLLearningCNT;
//         }

//         if(doLearning && _nWLLearningCNT == _nSym4IQEst)
//         {
//             // この時点でWLの推定が完了しているはず
//             // まずは，無線チャネルの周波数応答とIQインバランスの係数を推定する
//             learningCFRAndIQCoefs();

//             // この時の入力inputは2つ以上のシンボルである
//             assert(input.length >= nSym * 2);

//             // 後ろから2シンボル抽出し，それを使ってPAの係数を学習する
//             learningPACoefs(input[$ - nSym*2 .. $].dup, desired[$ - nSym*2 .. $].dup);
//         }
//     }
...
//         if(doLearning || _nWLLearningCNT < _nSym4IQEst)
//         {
//             _wlFilter.apply!(Yes.doLearning)(input, desired, errors);
//         }
//         else
//         {
//             // 学習が完了している
//             // まずは, inputをTXIQミキサに入力した信号を得る
//             _regenIQTX(input, _buffer4PA[0]);
//             // これをPAに通過させる
//             _regenPA(_buffer4PA[0 .. P]);
//             // _buffer4PAから_buffer4Regeneratorに変換する
//             foreach(i; 0 .. input.length) foreach(p; 0 .. P) _buffer4Regenerator[i][p] = _buffer4PA[p][i];
//             // チャネルを掛け合わせる
//             _regenerator.apply(_buffer4Regenerator, errors);
//             // RXIQインバランスを再現
//             _regenIQRX(errors, _buffer4PA[0]);
//             // 自己干渉除去する
//             foreach(i; 0 .. input.length)
//                 errors[i] = desired[i] - _buffer4PA[0][i];
//         }
//     }
...
//   private:
//     WLFilter _wlFilter;
//     immutable size_t _nSym4IQEst;
//     immutable size_t _nSC4IQEst;
//     Complex!real _iqTX, _iqRX;
//     Complex!real[P] _coefsAMP;
//     Complex!real[] _channelFreqResponse;
//     size_t _nWLLearningCNT;
//     C[][P] _buffer4PA;
//     C[P][] _buffer4Regenerator;


//     C[][2] getRegionOfPth(C[] input, size_t p)
//     {
//         static assert(0);
//     }


//     void convToFreqDomain(C[] input)
//     {
//         static assert(0);
//     }


//     void learningPACoefs(C[] input, C[] desired)
//     in{
//         assert(input.length == (_nFFT + _nCP) * _nOS * 2);
//         assert(desired.length == (_nFFT + _nCP) * _nOS * 2);
//     }
//     body{
//         immutable nSym = (_nFFT + _nCP) * _nOS;
//         immutable nCPX = _nCP * _nOS;

//         // 最初にIQインバランスフリー化する
//         inverseIQImbalance(input, desired);

//         // 前後2シンボルの和
//         convToFreqDomain(desired[nCPX .. nSym]);
//         convToFreqDomain(desired[nCPX+nSym .. $]);
//         foreach(i; 0 .. _nOS * _nFFT)
//             desired[nCPX + i] += desired[nCPX + nSym + i];

//         // 高次から順番に推定していく
//         foreach_reverse(p; 1 .. P){
//             foreach(i; 0 .. input.length){
//                 auto c = input[i];
//                 _buffer4PA[p][i] = c * c.sqAbs^^(p);
//             }

//             convToFreqDomain(_buffer4PA[p][nCPX .. nSym]);
//             convToFreqDomain(_buffer4PA[p][nCPX+nSym .. $]);
//             foreach(i; 0 .. _nOS * _nFFT)
//                 _buffer4PA[p][nCPX + i] += _buffer4PA[p][nCPX + nSym + i];

//             auto cfreH = getRegionOfPth(_channelFreqResponse, p);
//             auto distX = getRegionOfPth(_buffer4PA[p][nCPX .. nSym], p);
//             auto recvY = getRegionOfPth(desired[nCPX .. nSym], p);
//             C num = complexZero!C;
//             real den = 0;
//             foreach(i; 0 .. _nSC4IQEst){
//                 num += recvY[i] * conj(cfreH[i] * distX[i]);
//                 den += sqAbs(cfreH[i] * distX[i]);
//             }
//             _coefsAMP[p] = num / den;

//             // 除去する
//             foreach(i; 0 .. _nOS * _nFFT)
//                 desired[nCPX + i] -= distX[nCPX + i] * _channelFreqResponse[i] * _coefsAMP[p];
//         }
//     }


//     // 最初に荒く自己干渉無線チャネルとIQインバランス係数を推定する
//     void learningCFRAndIQ_1st()
//     {
//         Slice!(2, Complex!float*) mx = slice!(Complex!float)(2, _nSC4IQEst);
//         Complex!float[] y = new Complex!float(_nSC4IQEst);
//         foreach(i; 0 .. _nSC4IQEst/2){
//             mx[0, i] = _wlFilter._stateAdapter.states[i+1].weight[0, 0];
//             mx[1, i] = _wlFilter._stateAdapter.states[$-1-i].weight[0, 0].conj;
//             y[i] = _wlFilter._stateAdapter.states[i+1].weight[0, 1];
//         }
//         foreach(i; _nSC4IQEst/2 .. _nSC4IQEst){
//             mx[0, i] = _wlFilter._stateAdapter.states[$-1-i].weight[0, 0];
//             mx[1, i] = _wlFilter._stateAdapter.states[i+1].weight[0, 0].conj;
//             y[$-i-1] = _wlFilter._stateAdapter.states[$-1-i].weight[0, 1];
//         }

//         auto coefTxRx = leastSquareEstimate(mx, y);
//         _iqTX = coefTxRx[0];
//         _iqRX = coefTxRx[1];

//         _channelFreqResponse = new Complex!float[_nFFT * _nOS];
//         foreach(i; 0 .. _nFFT * _nOS)
//             _channelFreqResponse[i] = _wlFilter._stateAdapter.states[i].weight[0, 0];
//     }


//     //
//     void learniningCFRAndIQ_2nd()
//     {
//         // Slice!(2, Complex!float*) 
//     }
// }
