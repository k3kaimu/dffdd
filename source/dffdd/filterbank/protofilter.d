...
// double[] designByNguyen(size_t nchannel, size_t polyphaseTaps, size_t m1 = 0)
// {
//     alias F = double;
//     immutable size_t M = nchannel,
//                      m = polyphaseTaps,
//                      N = 2*(m*M + m1);

//     ContiguousMatrix!F P = computeP!F(nchannel, polyphaseTaps, m1);

//     auto proto = designRootRaisedCosine(nchannel, polyphaseTaps, 0.025);
//     proto = proto[0 .. $/2];

//     static
//     double valueAndGradOfQuadraticForm(Mat)(size_t dim, const(double)* h, Mat A, double* grad)
//     {
//         if(grad){
//             foreach(i; 0 .. dim) {
//                 grad[i] = 0;
//                 foreach(j; 0 .. dim)
//                     grad[i] += h[j] * (A[i, j] * A[j, i]);
//             }
//         }
...
//     static struct OptData
//     {
//         size_t M, m, m1;
//         ContiguousMatrix!F A;
//         double eqvalue;
//     }

//     OptData optdata;
//     optdata.M = nchannel;
//     optdata.m = polyphaseTaps;
//     optdata.m1 = m1;
//     optdata.A = P;
...
//     extern(C)
//     static nothrow @nogc
//     double nloptFunc(uint n, const(double)* h, double* grad, void* ptr)
//     {
//         OptData* data = cast(OptData*)ptr;
//         auto dst = valueAndGradOfQuadraticForm(n, h, data.A, grad) - data.eqvalue;
...
//     static
//     ContiguousMatrix!F makeSelectMatrix(size_t n, size_t k)
//     {
//         ContiguousMatrix!F mat = slice!F(n, n);
//         mat[] = 0;
...
//         immutable size_t n = 2*M*(m-l) + 2*m1-1;
//         auto S1 = makeSelectMatrix(m*M+m1, n),
//              S2 = makeSelectMatrix(m*M+m1, n-m*M);

//         d.A = slice!F(m*M+m1, m*M+m1);
//         if(l <= m - 1 && l >= floor((m+1)/2.0)){
//             d.A[] = S1;
//             d.eqvalue = 0;
//         }else if(l <= floor((m+1)/2.0)-1 && l >= 1){
//             d.A[] = 0;
//             d.A[] += S1;
//             d.A[] += S2.reversed!0;
//             d.A[] += S2.reversed!1;
//             d.eqvalue = 0;
//         }else if(l == m){
//             auto S3 = makeSelectMatrix(m*M+m1, m*M+m1-2);
//             d.A[] = 0;
//             d.A[] += S3.reversed!0;
//             d.A[] += S3.reversed!1;
//             d.eqvalue = 1.0L / (2 * M);
...
//     import std.conv : to;
//     nlopt_opt opt;
//     opt = nlopt_create(NLOPT_LD_SLSQP, proto.length.to!uint);
//     scope(exit)
//         nlopt_destroy(opt);		

//     nlopt_set_min_objective(opt, &nloptFunc, &optdata);
//     foreach(i; 0 .. m)
//         nlopt_add_equality_constraint(opt, &nloptFunc, &(condata[i]), 1e-5);

//     nlopt_set_xtol_rel(opt, 1e-5);

//     double minf;
//     if(nlopt_optimize(opt, proto.ptr, &minf) < 0){
//         throw new Exception("Filter optimization failed!");
//     }

//     //copy half
//     foreach(i; 0 .. proto.length)
//         proto[$-1-i] = proto[i];
...
// {
//     // import std.stdio;
//     // auto ng = designByNguyen(4, 12, 0);
//     // writeln(ng);
// }


// ContiguousMatrix!F computeP(F)(size_t M, size_t m, size_t m1)
// {
//     immutable F B1 = 1,
//               w11 = (PI/M) - 0.08 * (PI/M),
//               w12 = PI;

//     immutable size_t N = 2 * (m*M + m1);
//     auto P = slice!F(m*M + m1, m*M + m1);
//     foreach(ptrdiff_t k; 0 .. m*M+m1){
//         foreach(ptrdiff_t l; 0 .. m*M+m1) {
//             F i1, i2;
//             if(k - l == 0)
//                 i1 = w12 - w11;
//             else
//                 i1 = (sin(w12*(k-l)) - sin(w11*(k-l)))/(k-l);

//             if(N-1-k-l == 0)
//                 i2 = w12 - w11;
//             else
//                 i2 = (sin(w12*(N-1-k-l)) - sin(w11*(N-1-k-l)))/(N-1-k-l);
